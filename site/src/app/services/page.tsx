      ...
      <section className="border-b border-[--color-border] hero-bg">
        <div className="mx-auto max-w-[1400px] px-6 py-24 sm:px-10 sm:py-32">
          ...
          <h1 className="h-display max-w-[20ch]" style={{ fontSize: "clamp(2.6rem, 7vw, 5rem)" }}>
            Wound care, <span className="h-serif text-[--color-rose]">primary care</span>, and what your employer needs.
          </h1>
            ...
            {items.map((s, i) => (
              <Reveal key={s.id} delay={i * 0.05}>
                <div className="card card-hover p-7 h-full flex flex-col">
                  <h3 className="font-display text-xl text-[--color-plum] leading-tight">{s.name}</h3>
                  <p className="mt-3 text-[--color-muted] text-[14px] leading-relaxed mb-5">{s.short}</p>
                  <div className="mt-auto">
                    {s.slug ? (
                      <Link
                        href={`/services/${s.slug}`}
                        className="inline-flex items-center gap-2 text-[12px] font-mono uppercase tracking-[0.22em] text-[--color-rose-deep] hover:text-[--color-rose]"
                      >
                        Deep page →
                      </Link>
                    ) : (
                      <span className="inline-flex items-center gap-2 text-[12px] font-mono uppercase tracking-[0.22em] text-[--color-muted]">
                        Ask about this →
                      </span>
                    )}
                  </div>
      ...
      <section className="border-t border-[--color-border] bg-white">
        <div className="mx-auto max-w-[1100px] px-6 py-24 text-center sm:px-10">
          <Reveal>
            ...
            <Link
              href="/contact"
              className="mt-10 inline-flex items-center gap-3 bg-[--color-rose] text-white px-7 py-4 font-semibold rounded-full hover:bg-[--color-rose-deep] transition-colors"
            >
